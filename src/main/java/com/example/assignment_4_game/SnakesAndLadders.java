...
public class SnakesAndLadders extends Application {
    ...
    private Scene loginScene(Stage primaryStage) {
        ...
        codeLabel.setTranslateX(50);
        codeLabel.setTranslateY(150);
        codeLabel.setStyle("-fx-font-size: 14px; -fx-font-weight: bold; -fx-text-fill: white;");

        TextField codeInput = new TextField();
        codeInput.setPromptText("Game Code");
        codeInput.setTranslateX(150);
        codeInput.setTranslateY(150);
        codeInput.setStyle("-fx-background-radius: 10; -fx-padding: 5;");
        ...
        saveButton.setStyle("-fx-background-color: #4CAF50; -fx-text-fill: white; -fx-font-size: 14px; -fx-background-radius: 20;");
        saveButton.setOnMouseEntered(e -> saveButton.setStyle("-fx-background-color: #45A049; -fx-text-fill: white; -fx-font-size: 14px; -fx-background-radius: 20;"));
        saveButton.setOnMouseExited(e -> saveButton.setStyle("-fx-background-color: #4CAF50; -fx-text-fill: white; -fx-font-size: 14px; -fx-background-radius: 20;"));
        ...
        saveButton.setOnAction(e -> {
            ...
            if (player1Name.isEmpty() || player2Name.isEmpty() || code.isEmpty()) {
                Label invalid = new Label("All fields must be filled.");
                invalid.setTranslateX(150);
                invalid.setTranslateY(250);
                invalid.setTextFill(Color.RED);
                loginPane.getChildren().add(invalid);
            } else {
                savePlayerData(code, player1Name, player2Name);
                Label success = new Label("Sign-Up Successful!");
                success.setTranslateX(150);
                success.setTranslateY(250);
                success.setTextFill(Color.GREEN);
                loginPane.getChildren().add(success);
            }
        });
        ...
        loginButton.setStyle("-fx-background-color: #4CAF50; -fx-text-fill: white; -fx-font-size: 14px; -fx-background-radius: 20;");
        loginButton.setOnMouseEntered(e -> loginButton.setStyle("-fx-background-color: #45A049; -fx-text-fill: white; -fx-font-size: 14px; -fx-background-radius: 20;"));
        loginButton.setOnMouseExited(e -> loginButton.setStyle("-fx-background-color: #4CAF50; -fx-text-fill: white; -fx-font-size: 14px; -fx-background-radius: 20;"));

        loginButton.setOnAction(e -> {
            String code = codeInput.getText().trim();

            if (code.isEmpty()) {
                Label invalid = new Label("Code is required to login.");
                invalid.setTranslateX(150);
                invalid.setTranslateY(250);
                invalid.setTextFill(Color.RED);
                loginPane.getChildren().add(invalid);
            } else {
                if (verifyPlayerData(code)) {
                    primaryStage.setScene(new Scene(BuildScene(), 600, 700));
                    primaryStage.setTitle("Snakes and Ladders");
                } else {
                    Label invalid = new Label("Invalid code. Please sign up.");
                    invalid.setTranslateX(150);
                    invalid.setTranslateY(250);
                    invalid.setTextFill(Color.RED);
                    loginPane.getChildren().add(invalid);
        ...
        loginPane.getChildren().addAll(player1, input1, player2, input2, codeLabel, codeInput, saveButton, loginButton);
        return new Scene(loginPane, 400, 300);


    }

    private void savePlayerData(String code, String player1Name, String player2Name) {
        try (FileWriter fileWriter = new FileWriter("PlayerNames.txt", true)) {
            fileWriter.write("Code: " + code + "\n");
    ...
    private boolean verifyPlayerData(String code) {
        try (Scanner scanner = new Scanner(new File("PlayerNames.txt"))) {
            while (scanner.hasNextLine()) {
                String line = scanner.nextLine();
                if (line.startsWith("Code: ") && line.substring(6).trim().equals(code)) {
                    return true;
                }
            }
        } catch (IOException e) {
